-- AI Bible Chat Database Schema for Supabase
-- This schema creates tables to store AI Bible chat conversations and messages
...
-- Allow public read access to chat_categories (no RLS needed)
-- chat_categories table doesn't need RLS as it contains public category data
...
COMMENT ON TABLE chat_categories IS 'Predefined categories for AI Bible chat conversations';
COMMENT ON TABLE ai_conversations IS 'Individual chat conversations between users and AI';
COMMENT ON TABLE ai_messages IS 'Individual messages within conversations';
COMMENT ON TABLE user_chat_preferences IS 'User preferences for AI chat behavior';
COMMENT ON TABLE chat_analytics IS 'Analytics and feedback data for chat sessions';
